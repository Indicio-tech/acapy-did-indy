...
@pytest_asyncio.fixture
async def credential_with_status(controller: Controller, indy_did: str):
    """Produce a credential for testing."""
    vm = indy_did + "#assert"
    credential = {
        "credential": {
...
@pytest.mark.asyncio
async def test_didkit_verify_with_status(credential_with_status: dict, resolver: Callable, contexts: dict):
    """Test didkit verification of a credential."""
    result = wrapper.verify_credential(credential_with_status, {}, resolver, contexts)
    assert not result["warnings"]
    assert not result["errors"]
...
@pytest.mark.asyncio
@pytest.mark.xfail(reason="This credential fails to verify for some reason")
